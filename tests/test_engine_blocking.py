...
class TestEngineBlocking:
    def test_engine_stalls_until_output_online(self, temp_ipc_paths):
        """
        Verify that the engine stalls (blocks) when the output socket is not ready,
        and resumes once it becomes ready.
        """
        ...
        try:
            ...
            # processed a subsequent message or that the message hasn't disappeared if we were using a different protocol.
            # But primarily, we want to ensure that NO message is dropped.
